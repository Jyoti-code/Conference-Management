...
export default function DelegateEdit() {
    const { id } = useParams();
    ...
    useEffect(() => {
        const fetchOrganizerDetails = async () => {
            try {
                const response =
                    await axios.get(`http://127.0.0.1:8000/api/delegateEdit/${id}`);
                setFormData(response.data.delegatesedit);
                console.log(response.data.delegatesedit)
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            ...
            setTimeout(() => {
                navigate('/conferences-past-list');
            }, 2000);
        } catch (error) {
            ...
            <div className='card' style={{ backgroundColor: '#fff', padding: '20px', borderRadius: '10px', boxShadow: '0 8px 10px rgba(0,0,0,0.1)', borderColor: '#fff' }}>
                <div className='card-body'>
                    <form method="post" onSubmit={handleSubmit}>
                        ...
                        <h3 className='d-flex justify-content-center h3 mb-0 text-gray-800'>Conference Details</h3>

                        <label>Conference Category <span style={{ color: '#F00' }}>*</span></label>
                        <select name="conference_category" id="conference_category" className="form-control input-lg" required onChange={handleChange} value={formData.conference_category}
                        >
                        ...
                        <div className='row'>
                            <div className='col-md-6'>
                                ...
                                <input type="date" name="from_date" id="from_date" className="dropdate"
                                    onChange={handleChange} value={formData.from_date} />
                            </div>
                            <div className='col-md-6'>
                                <div className="clearfix"></div>
                                <label >To Date<span style={{ color: '#F00' }}>*</span></label>
                                <input type="date" name="to_date" id="to_date" onChange={handleChange} value={formData.to_date} className="dropdate" /></div>
                        </div>
                        <div className='row'>
                            <div className='col-md-6'>
                                <div className="clearfix"></div>
