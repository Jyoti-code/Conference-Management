...
useEffect(() => {
    const fetchSubmitAbstract = async () => {
        const postData = {
            upload_file_hidden: formData.upload_file,
        };
        console.log("post data", postData)
        try {
            const response =
                await axios.get(`http://127.0.0.1:8000/api/abstractEdit/${id}`);
            setFormData(response.data.abstractedit[0]);
            if (response.data.abstractedit[0] && response.data.abstractedit[0].upload_file) {
                setPdfPreview(`http://127.0.0.1:8000/uploads/${response.data.abstractedit[0].upload_file}`);
            }
        } catch (error) {
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            const response = await axios.post('http://127.0.0.1:8000/api/abstractUpdate', formDataToSend);
                        ...
                        <div className='card-header py-3' style={{ position: 'sticky', top: 0, zIndex: 1 }}>
                            <div className='row'>
                                <div className='col-md-6'>
                                    <h6 className="m-0 font-weight-bold text-primary"><i className="fa fa-shield mr-3"></i>Update Abstract</h6>
                                </div>
                        ...
                        <div className='card-body' style={{ overflowY: 'auto' }}>
                            {formData && (
                                <form encType="multipart/form-data" onSubmit={handleSubmit}>
                                    ...
                                    <input type='file' className='form-control input-lg mb-3' name="upload_file" value={formData.upload_file.name} id="upload_file" onChange={handleChange} />
                                    ...
                                    <br />
                                    <button type='submit' className='btn btn-primary' name="upload_file">Update</button>
                                </form>
                            )}
                        </div>
                    </div>
